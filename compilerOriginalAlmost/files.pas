...
{<<<}
procedure opentemp;
{ Open all temp files needed by the compiler.
  These files are left open throughout the compilation of the program.
  The macro routine 'doheap' is responsible for maximizing the heap space available for each pass,
  without destroying these temp files.

  'Cache' is the software virtual memory work file (used by analys, travrs, and code).
  'Tempfileone' is used as scanner output (read by analys) and travrs output (read by code).
  'Tempfiletwo' is used as analyzer output (read by travrs).

  This minimizes the disk space required for compilation, and the amount of time needed for opening
...
{ Open files for parser. }
  ...
  begin {opena}
    if not scanalys then 
      reset(tempfileone); {allow analys to read the output from scan}
    ...
    if newdebugger and (switcheverplus[debugging] or switcheverplus[profiling]) then
...
{>>>}
{<<<}
procedure stf_pc {where: integer; (record to patch) pc: integer (new pc)} ;
