...
{ Environment file handling.  The state of analys is saved/restored by these two routines. }
{<<<}
procedure awriteenv;
...
{ Map a compiler index to debugger record index when
  we know an entry has already been made, as in a name. }
...
procedure gettyp (follow: tokenset; var resulttype: index); forward;
{<<<}
...
{ Syntactic routine to parse a type.
  Productions:
  type-denoter = ( [ "packed" ] structured-type ) | simple-type | pointer-type  .
